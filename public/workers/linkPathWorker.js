...
function segmentIntersectsAABB2D(p0, p1, min, max) {
  let tmin = 0;
  let tmax = 1;
  for (let i = 0; i < 2; i++) {
    const idx = i === 0 ? 0 : 2; // x,z
    const s = p0[idx] || 0;
    const e = p1[idx] || 0;
    const d = e - s;
    if (Math.abs(d) < 1e-8) {
      if (s < min[idx] || s > max[idx]) return false;
    } else {
      const inv = 1 / d;
      let t1 = (min[idx] - s) * inv;
      let t2 = (max[idx] - s) * inv;
      if (t1 > t2) {
        const tmp = t1; t1 = t2; t2 = tmp;
      }
      tmin = Math.max(tmin, t1);
...
function findTidyRoute(l, points, tidyNodes, byId) {
  if (!tidyNodes || !tidyNodes.length || !Array.isArray(points) || points.length < 2) return null;
  let best = null;
  let bestDist = Infinity;
  const a = byId?.get?.(l?.from);
  const b = byId?.get?.(l?.to);
  const forceForRack = !!(a?.rackId || b?.rackId);
  for (const tidy of tidyNodes) {
    const center = tidy.position || [0, 0, 0];
    const offset = tidy.tidy?.offset || {};
    const wallCenter = [
      (center[0] || 0) + (offset.x || 0),
      (center[1] || 0) + (offset.y || 0),
      (center[2] || 0) + (offset.z || 0),
    ];
    const spread = Number(tidy.tidy?.spread ?? 0) || 0;
    const spreadAngle = hashAngle(l?.id || "");
    const spreadV = spread
      ? [Math.cos(spreadAngle) * spread, 0, Math.sin(spreadAngle) * spread]
      : [0, 0, 0];
    const routePoint = [
      (wallCenter[0] || 0) + spreadV[0],
      (wallCenter[1] || 0) + spreadV[1],
      (wallCenter[2] || 0) + spreadV[2],
    ];

    const boxes = [];
    const v = tidy.tidy?.vertical || {};
    const h = tidy.tidy?.horizontal || {};
    const pad = 0.02;
    if (v.w && v.h && v.d) {
      boxes.push([
        [wallCenter[0] - v.w * 0.5 - pad, wallCenter[1] - v.h * 0.5 - pad, wallCenter[2] - v.d * 0.5 - pad],
        [wallCenter[0] + v.w * 0.5 + pad, wallCenter[1] + v.h * 0.5 + pad, wallCenter[2] + v.d * 0.5 + pad],
      ]);
    }
    if (h.w && h.h && h.d) {
      boxes.push([
        [wallCenter[0] - h.w * 0.5 - pad, wallCenter[1] - h.h * 0.5 - pad, wallCenter[2] - h.d * 0.5 - pad],
        [wallCenter[0] + h.w * 0.5 + pad, wallCenter[1] + h.h * 0.5 + pad, wallCenter[2] + h.d * 0.5 + pad],
      ]);
    }
    let hit = forceForRack || (tidy.tidy?.forceAll === true);
    for (let i = 0; i < points.length - 1 && !hit; i++) {
      const a = points[i];
      const b = points[i + 1];
      for (const box of boxes) {
        if (segmentIntersectsAABB(a, b, box[0], box[1]) || segmentIntersectsAABB2D(a, b, box[0], box[1])) {
          hit = true;
          break;
        }
      }
    ...
    if (!hit) continue;
    const dx = (center[0] || 0) - (points[0][0] || 0);
    const dy = (center[1] || 0) - (points[0][1] || 0);
    const dz = (center[2] || 0) - (points[0][2] || 0);
...
self.onmessage = (e) => {
  ...
  const tidyNodes = nodes.filter((n) => n && String(n.kind || "").toLowerCase() === "tidy" && n.tidy?.enabled);
  ...
  links.forEach((l) => {
    ...
    const selection = (() => {
      ...
      if (l.flowAnchorSetOwnerId) {
        ...
        const preferred = byId.get(ownerKey);
        if (preferred) {
    ...
    const dir = (selection.owner === b || selection.owner?.rackId === l.to) ? "in" : "out";
  ...
  for (const l of links) {
    ...
    const ownerOverride = l.flowAnchorSetOwnerId ? byId.get(String(l.flowAnchorSetOwnerId)) : null;
    const anyNoDiagonal =
      flowAnchorNoDiagonal === true ||
      getNoDiagonalForNode(a) ||
      getNoDiagonalForNode(b) ||
      getNoDiagonalForNode(ownerOverride) ||
      nodeHasNoDiagonalSet(a) ||
      nodeHasNoDiagonalSet(b) ||
      nodeHasNoDiagonalSet(ownerOverride);
    const anyBend90 =
      flowAnchorGlobalBend === 90 ||
      getGlobalBendForNode(a) === 90 ||
      getGlobalBendForNode(b) === 90 ||
      getGlobalBendForNode(ownerOverride) === 90 ||
      nodeHasBend90Set(a) ||
      nodeHasBend90Set(b) ||
      nodeHasBend90Set(ownerOverride) ||
    ...
    const tidyPoint = findTidyRoute(l, basePoints, tidyNodes, byId);
    let points = basePoints;
    if (tidyPoint) {
      const splitIndex = flowAnchorPoints.length > 0 ? (1 + flowAnchorPoints.length) : 1;
      points = basePoints.slice(0, splitIndex).concat([tidyPoint], basePoints.slice(splitIndex));
    }
