...
function normalizeAnchorBendDeg(v) {
  const n = Number(v) || 0;
  if (n <= 0) return 0;
  return n >= 67.5 ? 90 : 45;
}
...
function applyFlowAnchorBends(points, anchorBendsByIndex) {
  ...
  for (let i = 1; i < points.length - 1; i++) {
    const bendDeg = normalizeAnchorBendDeg(anchorBendsByIndex.get(i));
    if (bendDeg <= 0) {
      out.push(points[i]);
      continue;
    }
...
function segmentDetourCost(a, b, c) {
...
function findTidyRoute(points, tidyNodes) {
  ...
  for (const tidy of tidyNodes) {
    ...
    const forceAll = tidy.forceAll === true;
    if (forceAll) {
...
self.onmessage = (e) => {
  ...
  for (const l of links) {
    ...
    const flowAnchorGlobalBend = normalizeAnchorBendDeg(
    ...
    for (const anchor of flowAnchors) {
      ...
      const bendDeg = normalizeAnchorBendDeg(anchor?.bendDeg ?? flowAnchorGlobalBend ?? 0);
    ...
    const nodeHasBend90Set = (node) => {
      if (!node) return false;
      const sets = Array.isArray(node?.flowAnchorSets) ? node.flowAnchorSets : [];
      return sets.some((s) => normalizeAnchorBendDeg(s?.globalBendDeg ?? 0) === 90);
    };
    const nodeHasBendRuleSet = (node) => {
      if (!node) return false;
      const sets = Array.isArray(node?.flowAnchorSets) ? node.flowAnchorSets : [];
      return sets.some((s) => normalizeAnchorBendDeg(s?.globalBendDeg ?? 0) > 0);
    };
    const getGlobalBendForNode = (node) => {
      if (!node) return 0;
      const active = findActiveSet(node);
      if (active?.globalBendDeg != null) return normalizeAnchorBendDeg(active.globalBendDeg);
      return normalizeAnchorBendDeg(node?.flowAnchorGlobalBendDeg ?? 0);
    };
    ...
    const anyBendRule =
      ...
      getGlobalBendForNode(ownerOverride) > 0 ||
      nodeHasBendRuleSet(a) ||
      nodeHasBendRuleSet(b) ||
      nodeHasBendRuleSet(ownerOverride) ||
      Array.from(anchorBendsByIndex.values()).some((v) => v > 0);
    const forceOrthogonal = anyNoDiagonal || anyBend90;
    const forceStraight = forceOrthogonal || anyBendRule;
    ...
    let points = applyFlowAnchorBends(basePoints, anchorBendsByIndex);
    const tidyRoute = findTidyRoute(points, tidyNodes);
    if (tidyRoute?.point) {
    ...
    results.push({ id: l.id, points, forceOrthogonal, forceStraight });
  }
